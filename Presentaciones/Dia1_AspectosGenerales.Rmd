...
**MD5 (Message Digest 5)** es un algoritmo de resumen criptográfico que genera una cadena de 32 caracteres hexadecimales.

Características principales:

- **Longitud fija:** siempre produce un valor de 128 bits (32 caracteres hexadecimales).

- **Determinístico:** el mismo archivo o texto siempre genera el mismo hash.

- **Función resumen:** actúa como una “huella digital” del contenido.

- **Uso común:** verificar *integridad* de archivos descargados, transferidos, detectar cambios o corrupción en datos.

.center[.content-box-blue[
Si el *hash* calculado con `md5sum` coincide con el valor esperado (publicado por el autor del archivo), significa que el archivo está intacto.
]]
...
## Ejemplo: Datos transcriptomicos provenientes de *Arabidopsis thaliana* (At)

- ¿Qué significa **L7** en el nombre de las muestras?
- ¿Qué significa **_1 y _2**?

```{r, echo=FALSE, out.width='40%', fig.align='center'}
knitr::include_graphics("figures/Figura15.png")
```

En este ejemplo, cada carpeta `KO*` contiene:

- Archivos comprimidos `(*.gz)` con los transcriptomas.
- Un archivo `MD5.txt` con los valores esperados.

---

## Opción 1: Verificar la correcta descarga de los datos de manera manual

Calcula los *hashes MD5* de todos los archivos `.gz` en las carpetas `KO*`.

```{bash, eval=F}
md5sum KO*/*gz
```

Comparar esos valores con los que aparecen en cada **MD5.txt**:
...
## Opción 2: Verificar la correcta descarga de los datos directamente

Si empleas `md5sum` de esta manera:

```{bash, eval=F}
md5sum -c KO*/MD5.txt
```

¿Qué hace de esta manera?

- Lee directamente los valores esperados desde cada **MD5.txt**.

- Calcula el hash de los archivos listados y los compara automáticamente.

- Te dice si cada archivo está **OK** o **FAILED**, sin necesidad de revisar a mano.

- Es la forma más práctica cuando ya tienes los archivos **MD5.txt** proporcionados junto con los datos.

Ejemplo de salida:

```
KO1/archivo1.gz: OK
KO2/archivo2.gz: FAILED
...
## Tarea para el **16 de abril 2026**
...
- **Bioprojects previamente utilizados:** PRJNA821620 (At), PRJNA256121 (At), PRJNA858106 (Hs), PRJNA826506 (Hs), PRJNA649971 (Mm), PRJNA743296 (Hs), PRJNA739157 (Hs), PRJNA983389 (Hs), PRJNA759864 (Hs), PRJNA152985 (Dm), PRJNA494527 (Hs), PRJNA1198988 (Hs).
...
```{bash, eval=F}
cd /mnt/data/bioinfo-estadistica-2/RNAseq_2026/
ls
```
...
Crear una nueva carpeta que se llame "BioProject_2026" y que contenga dos carpetas internas: `data/` y `graph/`.

```{bash, eval=F}
cd data
mkdir BioProject_2026 
```
...
```{bash, eval=F}
BioProject_2026/ # directorio de trabajo donde nos localizamos
|- data/
|- counts/
```
...
```{bash, eval=F}
rsync ecoss@liigh.unam.mx:"/mnt/data/bioinfo-estadistica-2/RNAseq_2026/BioProject_2026/test.txt" .
```

Subir Información

```{bash, eval=F}
rsync ./download_all_rawData.sh ecoss@liigh.unam.mx:"/mnt/data/bioinfo-estadistica-2/RNAseq_2026/BioProject_2026/scripts"
```
