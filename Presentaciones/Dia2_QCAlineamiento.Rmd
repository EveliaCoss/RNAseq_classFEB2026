...
# 5. Pipelines de kallisto: Ensamblaje de **transcriptoma guiado**
...
```{bash, eval = F}
# Single-end
for file in ./data/processed//*trimmed.fq.gz
do
  clean=$(echo $file | sed 's/^.\{15\}//g;s/_trimmed//;s/\.fastq\.gz//;s/\.fq\.gz//')   # Nombre de la carpeta de salida, mismo nombre de SRA
  kallisto quant --index ./kallisto_quant/transcripts.idx --output-dir ./kallisto_quant/${clean} --single -l 200 -s 20  --threads=12 $file
done
...
## Script y ejecución en el cluster
  ...
  - [`kallisto_align.sh`](https://github.com/EveliaCoss/RNAseq_classFEB2026/blob/main/Practica_Dia2/scripts/trimm_adapters.sh)
...
# 6. Alineamiento con el genoma de referencia mediante **STAR**
...
```{bash, eval=FALSE}
ln -s /mnt/data/bioinfo-estadistica-2/RNAseq_2026/BioProject_2026/reference/GCF_000001405.40_GRCh38.p14_genomic.gtf.gz .
```
]
